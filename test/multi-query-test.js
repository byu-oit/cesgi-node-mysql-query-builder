...
describe('MySQL Multi-Statement Builder', function () {
  ...
  describe('Chained statements', function () {
  ...
  describe('Multiple statements per query builder', function () {
    it ('should form the select, delete, insert, and update clauses and parameters', function () {
      const tableName = 'persons'
      const condition = {id: 1}
      const resource = {name: 'John', age: 25}
      Sql.SELECT('*').FROM(tableName).WHERE(condition)
      Sql.DELETE(tableName).WHERE(condition)
      Sql.INSERT(tableName).VALUES(resource)
      Sql.UPDATE(tableName).SET(resource)
      const {queries, sql, params} = Sql
